...
# Both pinned KSU families already have stable helpers with exactly the domain
# semantics SUSFS needs.  Reuse them instead of importing newer SUSFS SID state.
s = selinux.read_text()
if "bool susfs_is_current_ksu_domain(void)" not in s:
    anchor = "\nvoid escape_to_root_for_adb_root(void)\n"
    if s.count(anchor) != 1:
        raise SystemExit(
            f"{selinux}: SELinux bridge anchor count={s.count(anchor)}"
        )
    bridge = r'''
...
# SUSFS 1.5.5 deliberately checks the mount's KSU devname before unmounting
# system partitions.  Keep that safety behavior; do not turn this into a blind
# path_umount wrapper.
s = umount.read_text()
