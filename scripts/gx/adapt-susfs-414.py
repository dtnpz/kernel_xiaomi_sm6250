...
# 1) fs/namespace.c: this vendor tree lacks the newer fslog/slub include
# context used by the generic SUSFS patch.  Insert the same SUSFS declarations
# after the last common scheduler include instead.
...
# 2) fs/proc/task_mmu.c: mm_inline.h is not present at the generic patch's
# include anchor in this tree.  Put the SUSFS definition include after uaccess.
...
# 3) fs/readdir.c: Android/OpenELA added dirent-name validation, so two SUSFS
# filters no longer match the old context.  Insert them at the equivalent
# semantic locations.
...
# Re-find compat function after the previous insertion shifted offsets.
compat_pos = s.find("static int compat_filldir(struct dir_context *ctx, const char *name, int namlen,")
...
# 4) fs/notify/fdinfo.c: this Android tree already switched inotify output to
# inotify_mark_user_mask() and ignored_mask:0.  Preserve that behavior while
# adding SUSFS's alternate inode/path reporting for suspicious marks.
...
# 5) kernel/sys.c: Miatoll already spoofs uname for Android bpf loaders.  Keep
# that compatibility path and run SUSFS spoofing after it so the SUSFS request
# is the final override for eligible processes.
...
# Remove only the six obsolete hunks represented by the exact transforms above.
# (Five files; readdir has two failed hunks.)
