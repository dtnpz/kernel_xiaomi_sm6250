...
if [[ "$GX_ROOT" == "xxksu" ]]; then
  [[ -f scripts/gx/strip-xxksu-legacy-vendor-hooks.py ]] || { echo "xxKSU legacy vendor-hook cleanup helper missing." >&2; exit 3; }
  python3 scripts/gx/strip-xxksu-legacy-vendor-hooks.py
fi

if [[ "$GX_SUSFS" == 1 ]]; then
  bash scripts/gx/setup-susfs.sh "$GX_ROOT"
  [[ -f scripts/gx/add-susfs-v155-bridges.py ]] || { echo "SUSFS v1.5.5 ABI bridge helper missing." >&2; exit 3; }
  case "$GX_ROOT" in
